...
void backward(int y, double** weight, double** bias, double** neuron, double** grad_weight,
    ...
    for (int i = output_layer_idx-1; i >= 0; --i) {
        ...
        for (int j = 0; j < width; j++) {
            if (neuron[i][j] > 0) {
                grad_neuron[i][j] = cblas_ddot(height, grad_neuron[i+1], 1, weight[i]+j, width);
            } else {
                grad_neuron[i][j] = 0;
            }
        }

        cblas_dger(CblasRowMajor, height, width, 1.0, grad_neuron[i+1], 1, neuron[i], 1, grad_weight[i], width);

        cblas_daxpy(height, 1.0, grad_neuron[i+1], 1, grad_bias[i], 1);
    }
}
