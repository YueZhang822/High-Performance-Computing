...
int forward(float* image, int image_size, int label, float** weight, float** bias,
            float** neurons, int hidden_size, int num_layers, float* loss, int thread_num, cublasHandle_t handle) {
    ...
    for (int i = 0; i < num_layers-1; i++) {
        ...
        fc_layer(neurons[i], neurons[i+1], weight[i], width, height, bias[i], handle);
        if (i < num_layers-2) {
...
void backward(int y, float** weight, float** bias, float** neuron, float** grad_weight,
            float** grad_bias, float** grad_neuron, int hidden_size, int num_layers, int thread_num, cublasHandle_t handle) {
    // output layer
    ...
    for (int i = output_layer_idx-1; i >= 0; --i) {
        ...
        assert(cublasSger(handle, width, height, &alpha, neuron[i], 1, grad_neuron[i+1], 1, grad_weight[i], width) == CUBLAS_STATUS_SUCCESS);

        assert(cublasSaxpy(handle, height, &alpha, grad_neuron[i+1], 1, grad_bias[i], 1) == CUBLAS_STATUS_SUCCESS);
    }
}
...
void train(float** train_data, int* train_label, int train_size, float** weight, float** bias,
            ...
            int batch_size, float* loss, float* accuracy, int thread_num, cublasHandle_t handle) {
    ...
    for (int e = 0; e < epochs; ++e) {
        ...
        for (int j = 0; j < train_size; j += batch_size) {
            ...
            for (int i = j; i < j + batch_size; i++) {
                ...
                int y = forward(train_data_cu[idx], IN_DIMENSION, train_label[idx], weight_cu, bias_cu, neurons, hidden_size, num_layers, &loss, thread_num, handle);
                acc_cnt += (y == train_label[idx]);
                backward(train_label[idx], weight_cu, bias_cu, neurons, grad_weight, grad_bias, grad_neuron, hidden_size, num_layers, thread_num, handle);
                loss_sum += loss;
...
void eval(float** test_data, int* test_label, int test_size, float** weight, float** bias,
            int hidden_size, int num_layers, float* loss, float* accuracy, int thread_num, cublasHandle_t handle) {
    float** neurons = NULL;
    ...
    for (int i = 0; i < test_size; ++i) {
        clear_neurons(neurons, num_layers, hidden_size);
        int y = forward(test_data_cu[i], IN_DIMENSION, test_label[i], weight_cu, bias_cu, neurons, hidden_size, num_layers, &loss_sum, thread_num, handle);
        cnt += (y == test_label[i]);
    }
...
int main(int argc, char** argv) {
    ...
    clock_t start, end;
    start = clock();
    train(train_set, train_labels, TRAINING_SET_SIZE, weight, bias, nh, nl, lr, ne, nb, &loss, &accuracy, thread_num, handle);
    end = clock();
    ...
    start = clock();
    eval(test_set, test_labels, TEST_SET_SIZE, weight, bias, nh, nl, &loss, &accuracy, thread_num, handle);
    end = clock();
