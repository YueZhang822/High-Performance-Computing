...
void fc_layer(float* in, float* out, float* w, int w_width, int w_height, float* bias, cublasHandle_t handle) {
    float alpha = 1.0;
    float beta = 1.0;
    assert(cudaMemcpy(out, bias, w_height * sizeof(float), cudaMemcpyDeviceToDevice) == cudaSuccess);
    assert(cublasSgemv(handle, CUBLAS_OP_T, w_width, w_height, &alpha, w, w_width, in, 1, &beta, out, 1) == CUBLAS_STATUS_SUCCESS);
}
