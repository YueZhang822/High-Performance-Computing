...
__global__ void ray_tracing_kernel(float* G, vector L, float Wy, float Wmax, vector C, float R, int n_work, int n_grid, curandState *d_state, int* count) {
...
void save(float* G, int n_grid) {
    char filename[] = "results/gpu-sp/res.txt";
